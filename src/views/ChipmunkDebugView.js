...
export default class ChipmunkDebugView {
    ...
    constructor(space) {
        ...
        if (global.debug > 1 && this.constructor === ChipmunkDebugView) {
            Object.seal(this);
        }
    }
    ...
    resize(scale) {
        ...
        canvas.style.left = 0;//(global.width  - this.width )+'px';
        canvas.style.top = (global.height - this.height) + 'px';
...
function drawCircle(ctx, scale, point2canvas, c, radius) {
    c = point2canvas(c);
    ctx.beginPath();
...
function drawLine(ctx, point2canvas, a, b) {
    a = point2canvas(a);
    b = point2canvas(b);
...
function drawSpring(ctx, scale, point2canvas, a, b) {
    a = point2canvas(a);
    b = point2canvas(b);
