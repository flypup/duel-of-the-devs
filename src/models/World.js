...
export default class World {
    ...
    constructor() {
        ...
        if (global.debug > 1 && this.constructor === World) {
            Object.seal(this);
        }
    }
    ...
    initMapEntity(entityData) {
        ...
        const z = entityData.z;
        const EntityClass = global.entities[entityData.type];
    ...
    initSpawnPoint(spawnPointData) {
        const x = spawnPointData.x;
        const y = spawnPointData.y;
        const z = spawnPointData.z;
        const EntityClass = global.entities[spawnPointData.entityClass];
        const InputClass = global.inputs[spawnPointData.inputClass];
        ...
        const entity = new EntityClass().setPos(x, y, z).setInput(input);
        return entity;
    }
    ...
    initMapElement(element) {
        ...
        return new MapElement(element);
    }

    addWalls(left, top, right, bottom) {
    ...
    step(delta) {
        ...
        if (global.debug === 1) {
            traceTime('space.step');
        }
        this.space.step(delta / 1000);
        if (global.debug === 1) {
            traceTimeEnd('space.step');
        }
        for (i = this.entities.length; i--;) {
